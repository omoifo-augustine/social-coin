...
;; Contribute funds to community treasury
...
;; Submit proposal to modify protocol parameters
(define-public (submit-proposal
  ...
  (let ((new-proposal-id (+ (var-get proposal-counter) u1)))
    ...
    (map-set governance-proposals new-proposal-id {
      ...
      votes-against: u0,
      status: "active",
      expiry-height: (+ stacks-block-height u1440), ;; 10 days voting period
    })
...
;; Pause all reward claims (emergency function)
(define-public (pause)
  (begin
    (asserts! (is-contract-owner) err-owner-only)
    (var-set paused true)
